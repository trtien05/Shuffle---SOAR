...
export const findSpecificApp = (framework, inputcategory) => {
  ...
  if (
    ...
    if (
      framework["EDR & AV"] !== undefined &&
      framework["EDR & AV"].name !== undefined
    ) {
    ...
    if (
      framework["Comms"] !== undefined &&
      framework["Comms"].name !== undefined
    ) {
    ...
    if (
      framework["Email"] !== undefined &&
      framework["Email"].name !== undefined
    ) {
    ...
    if (
      framework["Assets"] !== undefined &&
      framework["Assets"].name !== undefined
    ) {
    ...
    if (
      framework["Cases"] !== undefined &&
      framework["Cases"].name !== undefined
    ) {
    ...
    if (
      framework["IAM"] !== undefined &&
      framework["IAM"].name !== undefined
    ) {
    ...
    if (
      framework["Network"] !== undefined &&
      framework["Network"].name !== undefined
    ) {
    ...
    if (
      framework["Intel"] !== undefined &&
      framework["Intel"].name !== undefined
    ) {
    ...
    if (
      framework["SIEM"] !== undefined &&
      framework["SIEM"].name !== undefined
    ) {
...
const AppFramework = (props) => {
  ...
  const [frameworkSuggestions, setFrameworkSuggestions] =
    React.useState([]);
  ...
  const [selectedUsecase, setSelectedUsecase] = React.useState(
    selectedOption !== undefined ? selectedOption : "Phishing"
  );
  ...
  const handleLoadNextSuggestion = (frameworkData) => {
    ...
    fetch(
      "https://europe-west2-shuffler.cloudfunctions.net/app_recommendations",
      {
        method: "POST",
        headers: {
    ...
    for (var key in usecaseTypes) {
      for (var subkey in usecaseTypes[key].value) {
        ...
        for (var itemtype in usecase.items) {
          var apptype =
            usecase.items[itemtype].app_type.toLowerCase();
          if (
            apptype.toLowerCase() === "email" ||
            apptype.toLowerCase() === "comms" ||
          ...
          if (
            changed.toLowerCase() === apptype.toLowerCase() ||
            ...
            if (
              frameworkData[apptype] !== undefined &&
              frameworkData[apptype].name !== undefined &&
              frameworkData[apptype].name !== null &&
              frameworkData[apptype].name.length > 0
            ) {
        ...
        if (potential) {
          ...
          if (
            inputUsecases !== undefined &&
            setInputUsecases !== undefined &&
            usecase.usecase_references !== undefined &&
            usecase.usecase_references.length > 0
          ) {
            ...
            for (var usecaseKey in inputUsecases) {
              ...
              for (var subUsecaseKey in usecaseCategory.list) {
                const loopUsecase =
                  usecaseCategory.list[subUsecaseKey];
                if (
                  loopUsecase.matches === undefined ||
                  loopUsecase.matches === null ||
                  loopUsecase.matches.length === 0
                ) {
                ...
                if (
                  usecase.usecase_references.includes(
                    loopUsecase.name
                  )
                ) {
                  foundUsecase = true;
            ...
            if (!foundUsecase) {
              ...
              console.log(
                "FOUND usecase existing in ",
                usecase.usecase_references
              );
              continue;
            }
          } else {
  ...
  useEffect(() => {
    ...
    if (
      discoveryWrapper === undefined ||
      discoveryWrapper.id === "SHUFFLE" ||
      discoveryWrapper.id === undefined ||
      cy === undefined
    ) {
      setDiscoveryData({});
    ...
    for (var key in nodes) {
      ...
      if (
        newSearchName === "ERADICATION" ||
        newSearchName === "ENDPOINT"
      ) {
    ...
    fetch(globalUrl + "/api/v1/apps/frameworkConfiguration", {
      ...
      .then((responseJson) => {
        if (responseJson.success === false) {
          if (responseJson.reason !== undefined) {
            toast(
              "Failed getting appframework: " + responseJson.reason
            );
          } else {
            toast("Failed to update framework for your org.");
          }
  ...
  useEffect(() => {
    ...
    if (
      newSelectedApp.objectID === undefined ||
      newSelectedApp.objectID === undefined ||
      newSelectedApp.objectID.length === 0
    ) {
      return;
    }
    ...
    const submitValue = {
      ...
      description:
        newSelectedApp.description === undefined
          ? ""
          : newSelectedApp.description,
    };
    ...
    if (setFrameworkData !== undefined) {
      ...
      if (keys.length === 0) {
        ...
        if (
          discoveryData.large_image !== undefined &&
          discoveryData.large_image !== null &&
          discoveryData.large_image.includes("storage.googleapis.com")
        ) {
  ...
  const imgSize = 50;
  var parsedFrameworkData =
    frameworkData === undefined ? {} : frameworkData;

  // Awful mapping to make sure all access is always there
  if (frameworkData !== undefined) {
    if (frameworkData.cases !== undefined) {
      if (
        (frameworkData.cases.large_image === undefined &&
          frameworkData.cases.large_image === null) ||
        frameworkData.cases.large_image === ""
      ) {
        frameworkData.cases = {};
      }
    ...
    if (frameworkData.siem !== undefined) {
      if (
        (frameworkData.siem.large_image === undefined &&
          frameworkData.siem.large_image === null) ||
        frameworkData.siem.large_image === ""
      ) {
        frameworkData.siem = {};
      }
    ...
    if (frameworkData.assets !== undefined) {
      if (
        (frameworkData.assets.large_image === undefined &&
          frameworkData.assets.large_image === null) ||
        frameworkData.assets.large_image === ""
      ) {
    ...
    if (frameworkData.intel !== undefined) {
      if (
        (frameworkData.intel.large_image === undefined &&
          frameworkData.intel.large_image === null) ||
        frameworkData.intel.large_image === ""
      ) {
        frameworkData.intel = {};
      }
    ...
    if (frameworkData.communication !== undefined) {
      if (
        (frameworkData.communication.large_image === undefined &&
          frameworkData.communication.large_image === null) ||
        frameworkData.communication.large_image === ""
      ) {
    ...
    if (frameworkData.network !== undefined) {
      if (
        (frameworkData.network.large_image === undefined &&
          frameworkData.network.large_image === null) ||
        frameworkData.network.large_image === ""
      ) {
    ...
    if (frameworkData.iam !== undefined) {
      if (
        (frameworkData.iam.large_image === undefined &&
          frameworkData.iam.large_image === null) ||
        frameworkData.iam.large_image === ""
      ) {
        frameworkData.iam = {};
      }
    ...
    if (frameworkData.edr !== undefined) {
      if (
        (frameworkData.edr.large_image === undefined &&
          frameworkData.edr.large_image === null) ||
        frameworkData.edr.large_image === ""
      ) {
        frameworkData.edr = {};
      }
  ...
  const changeUsecase = (value, type) => {
    ...
    for (var key in newedges) {
      ...
      if (
        newedges[key].description !== undefined &&
        newedges[key].description !== null &&
        newedges[key].description.length > 0
      ) {
        newedges[key].label =
          parseInt(key) + 1 + " " + newedges[key].description;
      }
  ...
  const onNodeSelect = (event) => {
    ...
    setDefaultSearch(
      data.label.charAt(0).toUpperCase() +
        data.label.substring(1).toLowerCase()
    );
  };
  ...
  const onEdgeSelect = (event) => {
    ...
    if (
      event.target.data("human") === undefined ||
      event.target.data("human") === false
    ) {
  ...
  if (
    ...
    const foundMiddleImage =
      userdata !== undefined &&
      userdata !== null &&
      userdata.active_org !== undefined &&
      userdata.active_org.image !== undefined &&
      userdata.active_org.image !== null &&
      userdata.active_org.image !== ""
        ? userdata.active_org.image
        : "/images/Shuffle_logo.png";

    const siemcheck =
      parsedFrameworkData.SIEM.large_image === undefined ||
      (parsedFrameworkData.SIEM.name !== undefined &&
        parsedFrameworkData.SIEM.name !== null &&
        parsedFrameworkData.SIEM.name.includes(":default"));
    const iamcheck =
      parsedFrameworkData.IAM.large_image === undefined ||
      (parsedFrameworkData.IAM.name !== undefined &&
        parsedFrameworkData.IAM.name !== null &&
        parsedFrameworkData.IAM.name.includes(":default"));
    const casescheck =
      parsedFrameworkData.Cases.large_image === undefined ||
      (parsedFrameworkData.Cases.name !== undefined &&
        parsedFrameworkData.Cases.name !== null &&
        parsedFrameworkData.Cases.name.includes(":default"));
    const assetscheck =
      parsedFrameworkData.Assets.large_image === undefined ||
      (parsedFrameworkData.Assets.name !== undefined &&
        parsedFrameworkData.Assets.name !== null &&
        parsedFrameworkData.Assets.name.includes(":default"));
    const intelcheck =
      parsedFrameworkData.Intel.large_image === undefined ||
      (parsedFrameworkData.Intel.name !== undefined &&
        parsedFrameworkData.Intel.name !== null &&
        parsedFrameworkData.Intel.name.includes(":default"));
    const commscheck =
      parsedFrameworkData.Comms.large_image === undefined ||
      (parsedFrameworkData.Comms.name !== undefined &&
        parsedFrameworkData.Comms.name !== null &&
        parsedFrameworkData.Comms.name.includes(":default"));
    const edrcheck =
      parsedFrameworkData["EDR & AV"].large_image === undefined ||
      (parsedFrameworkData["EDR & AV"].name !== undefined &&
        parsedFrameworkData["EDR & AV"].name !== null &&
        parsedFrameworkData["EDR & AV"].name.includes(":default"));
    const networkcheck =
      parsedFrameworkData.Network.large_image === undefined ||
      (parsedFrameworkData.Network.name !== undefined &&
        parsedFrameworkData.Network.name !== null &&
        parsedFrameworkData.Network.name.includes(":default"));
    ...
    const nodes = [
      {
        ...
        data: {
          ...
          name:
            parsedFrameworkData.Cases.name === undefined
              ? ""
              : parsedFrameworkData.Cases.name,
          description:
            parsedFrameworkData.Cases.description === undefined
              ? ""
              : parsedFrameworkData.Cases.description,
          app_id:
            parsedFrameworkData.Cases.id === undefined
              ? ""
              : parsedFrameworkData.Cases.id,
          text_margin_y: casescheck
            ? textMarginDefault
            : textMarginImage,
      ...
      },
      {
        ...
        data: {
          ...
          name:
            parsedFrameworkData.IAM.name === undefined
              ? ""
              : parsedFrameworkData.IAM.name,
          description:
            parsedFrameworkData.IAM.description === undefined
              ? ""
              : parsedFrameworkData.IAM.description,
          app_id:
            parsedFrameworkData.IAM.id === undefined
              ? ""
              : parsedFrameworkData.IAM.id,
          text_margin_y: iamcheck
            ? textMarginDefault
            : textMarginImage,
        ...
        renderedPosition: {
          x:
            baselocationX + shiftradius + shiftradius / shiftmodifier,
          y:
            baselocationY + shiftradius - shiftradius / shiftmodifier,
        },
      },
      {
        group: "nodes",
        data: {
          ...
          name:
            parsedFrameworkData.Assets.name === undefined
              ? ""
              : parsedFrameworkData.Assets.name,
          description:
            parsedFrameworkData.Assets.description === undefined
              ? ""
              : parsedFrameworkData.Assets.description,
          app_id:
            parsedFrameworkData.Assets.id === undefined
              ? ""
              : parsedFrameworkData.Assets.id,
          text_margin_y: assetscheck
            ? textMarginDefault
            : textMarginImage,
      ...
      },
      {
        ...
        data: {
          ...
          name:
            parsedFrameworkData.Intel.name === undefined
              ? ""
              : parsedFrameworkData.Intel.name,
          description:
            parsedFrameworkData.Intel.description === undefined
              ? ""
              : parsedFrameworkData.Intel.description,
          app_id:
            parsedFrameworkData.Intel.id === undefined
              ? ""
              : parsedFrameworkData.Intel.id,
          text_margin_y: intelcheck
            ? textMarginDefault
            : textMarginImage,
      ...
      },
      {
        ...
        data: {
          ...
          name:
            parsedFrameworkData.Comms.name === undefined
              ? ""
              : parsedFrameworkData.Comms.name,
          description:
            parsedFrameworkData.Comms.description === undefined
              ? ""
              : parsedFrameworkData.Comms.description,
          app_id:
            parsedFrameworkData.Comms.id === undefined
              ? ""
              : parsedFrameworkData.Comms.id,
          text_margin_y: commscheck
            ? textMarginDefault
            : textMarginImage,
      ...
      },
      {
        ...
        data: {
          ...
          name:
            parsedFrameworkData["EDR & AV"].name === undefined
              ? ""
              : parsedFrameworkData["EDR & AV"].name,
          description:
            parsedFrameworkData["EDR & AV"].description === undefined
              ? ""
              : parsedFrameworkData["EDR & AV"].description,
          app_id:
            parsedFrameworkData["EDR & AV"].id === undefined
              ? ""
              : parsedFrameworkData["EDR & AV"].id,
          text_margin_y: edrcheck
            ? textMarginDefault
            : textMarginImage,
      ...
      },
      {
        ...
        data: {
          ...
          name:
            parsedFrameworkData.Network.name === undefined
              ? ""
              : parsedFrameworkData.Network.name,
          description:
            parsedFrameworkData.Network.description === undefined
              ? ""
              : parsedFrameworkData.Network.description,
          app_id:
            parsedFrameworkData.Network.id === undefined
              ? ""
              : parsedFrameworkData.Network.id,
          text_margin_y: networkcheck
            ? textMarginDefault
            : textMarginImage,
      ...
      },
      {
        ...
        data: {
          ...
          name:
            parsedFrameworkData.SIEM.name === undefined
              ? ""
              : parsedFrameworkData.SIEM.name,
          description:
            parsedFrameworkData.SIEM.description === undefined
              ? ""
              : parsedFrameworkData.SIEM.description,
          app_id:
            parsedFrameworkData.SIEM.id === undefined
              ? ""
              : parsedFrameworkData.SIEM.id,
          text_margin_y: siemcheck
            ? textMarginDefault
            : textMarginImage,
    ...
    nodes.push({
      ...
      data: {
        ...
        errors: [],
        large_image:
          encodeURI(`data:image/svg+xml;utf-8,<svg fill="rgb(248,90,62)" width="${svgSize}" height="${svgSize}" viewBox="0 0 ${svgSize} ${svgSize}" version="1.1" xmlns="http://www.w3.org/2000/svg">
    ...
    if (inputUsecase !== undefined && inputUsecase !== null) {
      for (var key in inputUsecase.process) {
        if (
          inputUsecase.process[key].source === "" ||
          inputUsecase.process[key].target === ""
        ) {
          continue;
        }
  ...
  if (
    ...
    selectedUsecase !== selectedOption
  ) {
  ...
  const UsecaseHandler = (props) => {
    ...
    const leftImage =
      data.left_image !== undefined
        ? parsedDatatypeImages()[data.left_image.toUpperCase()]
        : undefined;
    const rightImage =
      data.right_image !== undefined
        ? parsedDatatypeImages()[data.right_image.toUpperCase()]
        : undefined;
    ...
    var svgIcon = (
      ...
      >
        ...
        />
        <path
          d="M4.21436 16L1.00007 13"
          stroke="#AFAFAF"
          stroke-linecap="round"
        />
        <path
          d="M4.21436 10L1.00007 13"
          stroke="#AFAFAF"
          stroke-linecap="round"
        />
      </svg>
    ...
    if (data.direction === "right") {
      svgIcon = (
        ...
        >
          <path
            d="M0.847656 4.16553H14.1479"
            stroke="#AFAFAF"
            stroke-linecap="round"
          />
          <path
            d="M11.8268 1.37085L14.8211 4.16556"
            stroke="#AFAFAF"
            stroke-linecap="round"
          />
          <path
            d="M11.8268 6.96024L14.8211 4.16553"
            stroke="#AFAFAF"
            stroke-linecap="round"
          />
        </svg>
      ...
      svgIcon = (
        ...
        >
          <path
            d="M14.5031 4.16577L1.20278 4.16577"
            stroke="#AFAFAF"
            stroke-linecap="round"
          />
          <path
            d="M3.52393 6.96045L0.529589 4.16573"
            stroke="#AFAFAF"
            stroke-linecap="round"
          />
          <path
            d="M3.52393 1.37106L0.529589 4.16577"
            stroke="#AFAFAF"
            stroke-linecap="round"
          />
        </svg>
    ...
    const handleHover = () => {
      ...
      for (var key in data.automated) {
        ...
        if (
          data.automated[key].description !== undefined &&
          data.automated[key].description !== null &&
          data.automated[key].description.length > 0
        ) {
          data.automated[key].label =
            parseInt(key) + 1 + " " + data.automated[key].description;
        }
    ...
    const bgColor =
      color === undefined || color === null || color.length === 0
  ...
  const selectedUsecases = [
    {
      name: "Sync tickets",
      description:
        "Do cool shit weeeee - this just ensure they are syncing",
      left_text: "Cases",
      right_text: "Cases",
      left_image: "Cases",
      right_image: "Cases",
  ...
  return (
    ...
    >
          ...
          {Object.keys(usecases).map((data, index) => {
            return (
              <Button
                ...
                color="primary"
                variant={
                  selectedUsecase === data ? "contained" : "outlined"
                }
                style={{ margin: 5 }}
                onClick={() => {
      ...
      {Object.getOwnPropertyNames(discoveryData).length > 0 ? (
        ...
        >
          ...
          >
            <IconButton
              ...
              onClick={(e) => {
                ...
                const foundelement = cy.getElementById(
                  discoveryData.id
                );
                if (
                  ...
                  foundelement.data(
                    "large_image",
                    parsedDatatypeImages()[
                      discoveryData.id.toUpperCase()
                    ]
                  );
          ...
          <div style={{ display: "flex" }}>
            {discoveryData.name !== undefined &&
            discoveryData.name !== null &&
            discoveryData.name.length > 0 ? (
              <div
                style={{
                  border: "1px solid rgba(255,255,255,0.2)",
                  borderRadius: 25,
              ...
              >
                <img
                  alt={discoveryData.id}
                  src={
                    newSelectedApp.image_url !== undefined &&
                    newSelectedApp.image_url !== null &&
                    newSelectedApp.image_url.length > 0
                      ? newSelectedApp.image_url
                      : discoveryData.large_image
                  }
            ...
            >
              ...
              discoveryData.name.length > 0
                ? discoveryData.name
                : newSelectedApp.name !== undefined &&
                  newSelectedApp.name !== null &&
                  newSelectedApp.name.length > 0
                ? newSelectedApp.name
      ...
      />
      {isCloud &&
      inputUsecase !== undefined &&
      inputUsecase !== null &&
      isLoggedIn === true &&
      Object.keys(inputUsecase).length > 0 ? (
        <Button
          color="secondary"
          variant={"outlined"}
