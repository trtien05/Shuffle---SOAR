...
export const triggers = [
  {
    ...
    long_description:
      "Execute a workflow with an unauthicated POST request",
    id: "",
  },
  {
    name: "Schedule",
...
export function SetJsonDotnotation(jsonInput, inputKey) {
  ...
  if (Array.isArray(jsonInput)) {
    for (var i = 0; i < jsonInput.length; i++) {
      jsonInput[i] = SetJsonDotnotation(
        jsonInput[i],
        inputKey + ".#"
      );
    }
...
const AngularWorkflow = (defaultprops) => {
  ...
  const [triggerAuthentication, setTriggerAuthentication] =
    React.useState({});
  ...
  const [editWorkflowDetails, setEditWorkflowDetails] =
  ...
  const [subworkflowStartnode, setSubworkflowStartnode] =
    React.useState("");
  const [leftViewOpen, setLeftViewOpen] = React.useState(
    isMobile ? false : true
  );
  const [leftBarSize, setLeftBarSize] = React.useState(
    isMobile ? 0 : 325
  );
  ...
  const [variableAnchorEl, setVariableAnchorEl] =
    React.useState(null);
  ...
  const [selectedTriggerIndex, setSelectedTriggerIndex] =
    React.useState({});
  ...
  const [executionRunning, setExecutionRunning] =
    React.useState(false);
  const [executionModalOpen, setExecutionModalOpen] =
    React.useState(false);
  ...
  const [workflowExecutionCount, setWorkflowExecutionCount] =
    React.useState(0);
  ...
  // For code editor
  const [codeEditorModalOpen, setCodeEditorModalOpen] =
    React.useState(false);
  ...
  const triggerEnvironments = isCloud
    ? ["cloud"]
    : ["onprem", "cloud"];
  const unloadText =
    "Are you sure you want to leave without saving (CTRL+S)?";
        ...
        if (responseJson.success === true) {
          if (
            ...
            if (
              ...
              const newdata = responseJson.reason.replace(
                imgRegex,
                "![]($1)"
              );
  ...
  useEffect(() => {
    if (
      ...
      console.log(
        `Should get app docs for: ${selectedAction.app_name}`
      );
      ...
      if (
        ...
        //
        const apptype =
          selectedApp.generated === false ? "python" : "openapi";
        getAppDocs(
  ...
  const getWorkflowExecutionCount = (workflowId) => {
    fetch(
      `${globalUrl}/api/v1/workflows/${workflowId}/executions/count`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
      ...
      .then((response) => {
        if (response.status !== 200) {
          console.log(
            "Status not 200 for workflow execution count: O!"
          );
          return;
        } else {
          return response.json();
        ...
        if (responseJson !== undefined) {
            ...
            if (trigger.parameters.length >= 3) {
              for (let paramkey in trigger.parameters) {
                ...
                if (
                  ...
                  if (
                    Object.getOwnPropertyNames(baseSubflow).length > 0
                  ) {
          ...
          if (workflows.length === 0) {
            ...
            if (parentworkflows.length > 0) {
              setParentWorkflows(
                parentworkflows.filter(
                  (wf) => wf.id !== props.match.params.key
                )
              );
            }
          }
    ...
    fetch(`${globalUrl}/api/v2/workflows/${id}/executions`, {
      ...
      .then((responseJson) => {
        if (
          ...
          var tmpView = new URLSearchParams(cursearch).get(
            "execution_id"
          );
          if (
          ...
          if (
            ...
            if (execution !== null && execution !== undefined) {
              if (
                execution.execution_argument.includes("too large")
              ) {
                setExecutionData({});
  ...
  const handleKafkaSubmit = (trigger) => {
    ...
    const bootstrapServers = document.getElementById(
      "bootstrap_servers"
    )?.value;
  ...
  const handleUpdateResults = (responseJson, executionRequest) => {
    ...
    ReactDOM.unstable_batchedUpdates(() => {
      if (
        JSON.stringify(responseJson) !== JSON.stringify(executionData)
      ) {
      ...
      if (
        responseJson.execution_id !== executionRequest.execution_id
      ) {
      ...
      if (
        ...
        for (let i = 0; i < curelements.length; i++) {
          if (
            curelements[i].classes().includes("executing-highlight")
          ) {
    ...
    for (let cyelementsKey in cyelements) {
      ...
      if (type === undefined) {
        ...
        if (type === "ACTION") {
          ...
          curworkflowAction.position =
            cyelements[cyelementsKey].position();
          ...
          if (
            ...
            if (
              cyelements[cyelementsKey].data().example !== undefined
            ) {
              curworkflowAction.example =
          ...
          for (let parametersKey in curworkflowAction.parameters) {
            const thisitem =
              curworkflowAction.parameters[parametersKey];
            if (
              thisitem.name.startsWith("${") &&
              thisitem.name.endsWith("}")
            ) {
              continue;
            }
    ...
    if (firstnode.length === 0) {
      ...
      for (let actionkey in workflow.actions) {
        if (workflow.actions[actionkey].isStartNode) {
          ...
          firstnode = cy.getElementById(
            workflow.actions[actionkey].id
          );
          found = true;
          break;
        }
    ...
    ReactDOM.unstable_batchedUpdates(() => {
      ...
      if (
        ...
        for (let actionkey in workflow.actions) {
          ...
          for (let paramkey in workflow.actions[actionkey]
            .parameters) {
            const param =
              workflow.actions[actionkey].parameters[paramkey];
            if (
              param.value === undefined ||
              param.value === null ||
              ...
              for (let authkey in tmpAuth) {
                ...
                if (itemname === appname) {
                  ...
                  if (item.edited > latest) {
                    ...
                    for (let actionkey in workflow.actions) {
                      const actionAppname = workflow.actions[
                        actionkey
                      ].app_name
                        .toLowerCase()
                        .replaceAll(" ", "_", -1);
  ...
  const getApps = () => {
    fetch(globalUrl + "/api/v1/apps", {
      ...
      .then((response) => {
        if (response.status !== 200) {
          console.log(
            "Status not 200 for apps. Setting to pretend apps!"
          );
          ...
          setApps(
            Array.prototype.concat.apply(pretend_apps, triggers)
          );
  ...
  const getFiles = () => {
    fetch(globalUrl + "/api/v1/files", {
      ...
      .then((responseJson) => {
        if (
          responseJson.files !== undefined &&
          responseJson.files !== null
        ) {
  ...
  const getWorkflow = (workflow_id, sourcenode) => {
    fetch(`${globalUrl}/api/v1/workflows/${workflow_id}`, {
          ...
          if (response.status >= 500) {
            ...
            const cursearch =
              ...
              window.location === undefined
                ? ""
                : window.location.search;
            var execFound = new URLSearchParams(cursearch).get(
              "execution_id"
            );
            ...
            if (execFound === null && sessionToken === null) {
              ...
              toast(
                `Injecting session token and reloading workflow..`
              );
              setTimeout(() => {
      ...
      .then((responseJson) => {
        ...
        if (
          responseJson.org_id !== undefined &&
          responseJson.org_id !== null
        ) {
    ...
    ReactDOM.unstable_batchedUpdates(() => {
      ...
      */
      if (
        ...
        const curaction = workflow.actions.find(
  ...
  const onNodeDragStop = (event, selectedAction) => {
    ...
    if (connected.length > 0 && connected !== undefined) {
      for (let connectkey in connected) {
        ...
        if (
          edge.data.decorator &&
          edge.data.label === releaseToConnectLabel
        ) {
    ...
    if (styledElements.length === 1) {
      ...
      if (curElement !== null && curElement !== undefined) {
        ...
        var newValue =
          "$" + nodedata.label.toLowerCase().replaceAll(" ", "_");
        ...
        if (idnumber >= 0 && paramname.length > 0) {
          ...
          const foundresult = GetParamMatch(
            parsedname,
            exampledata,
            ""
          );
        ...
        if (!found) {
          ...
          const matchingExecutions = workflowExecutions.filter(
            (execution) =>
              ...
              nodedata.app_name.toLowerCase()
          );
          console.log("Matches: ", matchingExecutions.length);
          const color =
            matchingExecutions.length > 0 ? "#34a853" : "#ea4436";
          const decoratorNode = {
            position: {
          ...
          for (var styledElementsKey in styledElements) {
            ...
            if (curElement !== null && curElement !== undefined) {
              curElement.style.border =
                curElement.style.original_border;
            }
          }
    ...
    ReactDOM.unstable_batchedUpdates(() => {
      if (data.isButton) {
        if (data.buttonType === "suggestion") {
          ...
          console.log(
            "Suggestion html to be added at location: ",
            event
          );
          /*
			const position = {
          ...
          if (
            ...
            var newNodeData = JSON.parse(
              JSON.stringify(parentNode.data())
            );
            ...
            const sourcebranches = workflow.branches.filter(
              (foundbranch) =>
                foundbranch.source_id === parentNode.data("id")
            );
            ...
            for (var destinationBranchesKey in destinationbranches) {
              var newbranch = JSON.parse(
                JSON.stringify(
                  destinationbranches[destinationBranchesKey]
                )
              );
      ...
      if (data.type === "ACTION") {
        ...
        //event.target.data()
        var curaction = workflow.actions.find(
          (a) => a.id === data.id
        );
        ...
        if (
          curaction.template === true &&
          curaction.name !== undefined
        ) {
          //newapps.
          const parsedname = curaction.name
            .replaceAll(" ", "_")
            .toLowerCase();
          console.log(
          ...
          for (var newAppskey in newapps) {
            for (let actionsSubkey in newapps[newAppskey].actions) {
              const tmpaction =
                newapps[newAppskey].actions[actionsSubkey];
              if (
                tmpaction.name.replaceAll(" ", "_").toLowerCase() ===
                parsedname
              ) {
            ...
            try {
              ...
              if (
                ...
                for (let appkey in parsedapps) {
                  if (
                    parsedapps[appkey].name === curaction.app_name
                  ) {
              ...
              console.log(
                "Problem with parsing apps from local storage",
                e
              );
            }
          } else {
		...
		*/
        ...
        if (!curapp || curapp === undefined) {
            ...
            setAuthenticationType(
              curapp.authentication.type === "oauth2-app" ||
                ...
                ? {
                    ...
                    client_secret:
                      curapp.authentication.client_secret,
            ...
            for (let tmpAuthKey in tmpAuth) {
              ...
              for (let fieldkey in item.fields) {
                ...
                if (filterkey === null || filterkey === undefined) {
                  console.log(
                    "Problem with filterkey 2. Null or undefined 3"
                  );
                  continue;
                }
          ...
          if (
            ...
            for (var curActionParamKey in curaction.parameters) {
              if (
                curaction.parameters[curActionParamKey].options !==
                  undefined &&
                curaction.parameters[curActionParamKey].options !==
                  null &&
                curaction.parameters[curActionParamKey].options
            ...
            console.log(
              "Should check APP if it has the same params as ACTION"
            );
            for (let actionKey in curapp.actions) {
              const tmpaction = curapp.actions[actionKey];
              if (tmpaction.name === curaction.name) {
                console.log(
                  "Found action - needs change?",
                  tmpaction
                );
                if (
        ...
        if (
          ...
          const envs = parsedenv.find(
            (a) => a.Name === curaction.environment
          );
        ...
        if (
          ...
          if (
            ...
            workflow.triggers[trigger_index].parameters[0] = {
              ...
              value:
                referenceUrl +
                "webhook_" +
                workflow.triggers[trigger_index].id,
            };

            if (
              workflow.triggers[trigger_index].parameters.length < 5
            ) {
          ...
          if (
            ...
            for (var envKey in environments) {
              ...
              if (
                environments[envKey].Name.toLowerCase() === "cloud"
              ) {
                continue;
              }
  ...
  //Ensuring that the results are normalized and valid before returning
  const GetExampleResult = (item) => {
    ...
    if (workflowExecutions.length > 0) {
      ...
      for (let execkey in workflowExecutions) {
        ...
        if (item.id === "exec") {
          if (
            workflowExecutions[execkey].execution_argument !==
              undefined &&
  ...
  const RunAutocompleter = (dstdata) => {
    ...
    if (parents.length > 1) {
      for (let parentkey in parents) {
        ...
        if (
          dstdata.parameters !== undefined &&
          dstdata.parameters !== null
        ) {
  ...
  const onEdgeAdded = (event) => {
        ...
        if (
          ...
          setTimeout(() => {
            ...
            if (targetedge !== -1) {
              ...
              toast.error(
                "Triggers can have exactly one target node"
              );
              event.target.remove();
              return;
    ...
    if (targetnode !== -1) {
      if (
        ...
        workflow.triggers[targetnode].app_name ===
          "Shuffle Workflow" ||
    ...
    if (eventTarget.data("isButton") === true) {
      const parentNode = cy.getElementById(
        eventTarget.data("attachedTo")
      );
      event.target.remove();
    ...
    const startNode = cy
      .nodes()
      .jsons()
      .find((node) => node.data.isStartNode === true);
    ...
    for (let branchkey in branches) {
      ...
      if (
        branch.target === edge.source &&
        branch.source === edge.target
      ) {
        ...
        branch.source === edge.source
      ) {
        if (branch.conditions === undefined) {
          // Edgehandles
        } else {
          console.log(
            "Removing because the same branch already exists"
          );
          event.target.remove();
  ...
  const onNodeAdded = (event) => {
    ...
    if (
      nodedata.decorator !== true &&
      nodedata.attachedTo === undefined
    ) {
  ...
  const onNodeRemoved = (event) => {
    ...
    workflow.actions = workflow.actions.filter(
      (a) => a.id !== data.id
    );
  ...
  const onNodeHoverOut = (event) => {
    ...
    const cytoscapeElement =
      document.getElementById("cytoscape_view");
  ...
  const addActionSuggestions = (nodedata, event) => {
    ...
    for (let appKey in apps) {
      const curapp = apps[appKey];
      if (
        curapp.name.toLowerCase().replace(" ", "_") !== parentname
      ) {
        continue;
      }
      ...
      for (let recIndex in rec.recommendations) {
        ...
        for (let branchKey in branches) {
          ...
          if (targetNode.data("name") === parsedRec.app_action) {
            console.log(
              "Found existing node (action name): ",
              targetNode
            );
            found = true;
          }
  ...
  const setupGraph = (inputworkflow) => {
    ...
    const foundtriggers = inputworkflow.triggers.map((trigger) => {
      ...
      if (
        ...
        var foundTrigger = triggers.find(
    ...
    if (
      ...
      const visualedges = inputworkflow.visual_branches.map(
        (branch, index) => {
          const edge = {};
  ...
  const fetchRecommendations = (inputWorkflow) => {
    console.log(
      "Disabled recommendations as they were too inaccurate"
    );
    return;
  };
  ...
  const getRevisionHistory = (workflow_id) => {
    fetch(`${globalUrl}/api/v1/workflows/${workflow_id}/revisions`, {
      ...
      .then((responseJson) => {
        ...
        if (responseJson.success === false) {
          console.log(
            "Error getting workflow revisions: ",
            responseJson
          );
          return;
        }
  ...
  if (firstrequest) {
    ...
    const tmpExec = new URLSearchParams(cursearch).get(
      "execution_highlight"
    );
    if (
      tmpExec !== undefined &&
  ...
  if (
    ...
    cy.on("boxend", (e) => {
      ...
      if (
        ...
        toast(
          `Selected ${cydata.length} element(s). CTRL+C to copy them.`
        );
      }
    });
  ...
  const submitSchedule = (trigger, triggerindex) => {
    ...
    const data = {
      ...
      execution_argument:
        workflow.triggers[triggerindex].parameters[1].value,
    ...
    const deleteVariable = (type, variableIndex) => {
      ...
      if (type === "normal") {
        if (
          ...
          var vars = JSON.parse(
            JSON.stringify(workflow.workflow_variables)
          );
        ...
        if (
          ...
          var vars = JSON.parse(
            JSON.stringify(workflow.execution_variables)
          );
          vars.splice(variableIndex, 1);
    ...
    return (
      <div style={appViewStyle}>
        <div style={variableScrollStyle}>
          ...
          {workflow.workflow_variables === null
            ? null
            : workflow.workflow_variables.map(
                (variable, varindex) => {
                  return (
                    <VariableItem
                      variable={variable}
          ...
          workflow.execution_variables === undefined
            ? null
            : workflow.execution_variables.map(
                (variable, varindex2) => {
                  return (
                    <VariableItem
                      variable={variable}
  ...
  const handleTriggerDrag = (e, data) => {
    ...
    if (
      ...
      if (newNodeId.length > 0) {
        ...
        currentnode[0].renderedPosition(
          "x",
          e.pageX - cycontainer.offsetLeft
        );
        currentnode[0].renderedPosition(
		...
		*/
        ...
        const newAppData = {
          ...
          required_body_fields:
            app.actions[actionIndex].required_body_fields,
          category:
            app.categories !== null &&
        ...
        if (
          !isCloud &&
          (!app.is_valid || (!app.activated && app.generated))
        ) {
  ...
  const AppView = (props) => {
    const { allApps, prioritizedApps, filteredApps, extraApps } =
      props;
    ...
    const [visibleApps, setVisibleApps] = React.useState(
      Array.prototype.concat.apply(
        ...
        Array.prototype.concat.apply(
          filteredApps.filter(
            (innerapp) =>
              !internalIds.includes(innerapp.id.toLowerCase())
          ),
          triggers
        )
      )
    ...
    const ParsedAppPaper = (props) => {
      ...
      newAppname =
        newAppname.charAt(0).toUpperCase() + newAppname.substring(1);
          ...
          {visibleApps.length > extraApps.length ? (
              ...
              {visibleApps.length <= 2 ? (
                ...
                >
                  <Typography variant="body1" color="textSecondary">
                    Click one of the relevant public apps below to
                    Activate it for your organization.
                  </Typography>
                  <InstantSearch
                ...
                >
                  <Typography variant="body1" color="textSecondary">
                    Apps need to be activated before they can be used.
                    Search from our 2500+ apps to activate them for
                    your organisation.
                  </Typography>
                </div>
              )}
            ...
            >
              <Typography variant="body1" color="textSecondary">
                ...
                unactivated apps. Click one of these apps to Activate
  ...
  const setNewSelectedAction = (e) => {
    ...
    newSelectedAction.required_body_fields =
      newaction.required_body_fields;
    ...
    if (
      ...
      for (let [paramkey, paramkeyval] in Object.entries(
        ...
        if (
          newSelectedAction.parameters[newParamIndex].name ===
          "headers"
        ) {
          if (
            param.value !== undefined &&
    ...
    if (newSelectedAction.app_name === "Shuffle Tools") {
      ...
      if (
        ...
        newSelectedAction.iconBackground =
          iconInfo.iconBackgroundColor;
      }
  ...
  const setTriggerFolderWrapperMulti = (event) => {
    ...
    if (
      (selectedTrigger.name === "Office365") &
        (value !== undefined) &&
      value !== null &&
      value.length > 1
    ) {
      ...
      <TextField
        ...
        helperText={
          ...
          data.value.includes(".#") ? (
            ...
            >
              Use "Shuffle Tools" app with "Filter List" action to
              handle loops
            </span>
          ) : null
        }
    ...
    >
      ...
      >
        <span style={{ color: "white" }}>
          Provide a runtime argument
        </span>
      </DialogTitle>
      <DialogContent>
        <Typography variant="body1" color="textSecondary">
          At least one node in this workflow requires a runtime
          argument ($exec). Please select one below, or provide a
          custom one in the text field next to the run button.
        </Typography>
        {/*
        ...
        <DialogContent style={{}}>
          <div style={{ display: "flex" }}>
            <Tooltip
              color="primary"
              title={
                conditionValue.configuration ? "Opposite" : "Default"
              }
              placement="top"
            >
              <span
          ...
          <Button
            ...
            onClick={() => {
              ...
              if (selectedEdge.conditions === undefined) {
                ...
                const curedgeindex =
                  selectedEdge.conditions.findIndex(
              ...
              if (selectedEdge.conditions.length === 1) {
                ...
                label =
                  selectedEdge.conditions.length + " conditions";
              }
  ...
  const EdgeSidebar = () => {
    ...
    if (
      ...
      injectedData = selectedEdge.conditions.map(
        (condition, index) => {
      ...
      <div style={appApiViewStyle}>
        ...
        {conditionsDisabled ? (
          <Typography variant="body1">
            Conditions are unavailable between triggers and the
            startnode.
          </Typography>
        ) : null}
              ...
              <span>
                ...
                {triggerFolders.length === 0 ? (
                  <Typography variant="body2">
                    No folders found. Please authenticate first. If
                    this persists,{" "}
                    <a
                      href="https://shuffler.io/contact"
                      rel="noopener noreferrer"
                  ...
                  <Select
                    ...
                    native
                    rows="10"
                    value={selectedTrigger.parameters[0].value.split(
                      splitter
                    )}
                    style={{
                      backgroundColor: theme.palette.inputColor,
  ...
  const handleWorkflowSelectionUpdate = (e, isUserinput) => {
    if (
      ...
      console.log(
        "Returning as there's no id. Value: ",
        e.target.value
      );
      return null;
    }
  ...
  const AppAuthSelector = ({ appAuthData }) => {
    ...
    const handleSelectChange = (appName, appId, event) => {
      ...
      if (
        ...
        if (authGroupValue.includes(appName)) {
          ...
          workflow.triggers[
            selectedTriggerIndex
          ].parameters[5].value = newValue;
        } else {
          workflow.triggers[
            selectedTriggerIndex
          ].parameters[5].value += appName + "=" + auth.id + ";";
        }
      }
    ...
    return (
      ...
      >
        {Object.entries(transformedAuthData).map(
          ([appId, authList]) => (
            <div
              key={appId}
              className="auth-item"
    ...
    if (actionlist.length === 0) {
      ...
      if (parents.length > 1) {
        for (let parentkey in parents) {
          ...
          var exampledata =
            item.example === undefined ? "" : item.example;
          ...
          if (workflowExecutions.length > 0) {
            ...
            for (let execkey in workflowExecutions) {
              ...
              var foundResult = workflowExecutions[
                execkey
              ].results.find(
                (result) => result.action.id === item.id
              );
      ...
      const handleItemClick = (values) => {
        ...
        */
        ...
        if (selectedTrigger.name === "Shuffle Workflow") {
          const toComplete =
            selectedTrigger.parameters[1].value +
            "$" +
            values[0].autocomplete;
      ...
      return (
        <div style={appApiViewStyle}>
          ...
          <FormControlLabel
            ...
            style={{ marginTop: 10 }}
            label={
              <div style={{ color: "white" }}>Wait for results</div>
            }
          />
          <div style={{ flex: "6", marginTop: 10 }}>
            <div>
              <div style={{ display: "flex" }}>
                ...
                {workflow.triggers[selectedTriggerIndex].parameters[0]
                  .value.length === 0 ? null : workflow.triggers[
                    selectedTriggerIndex
                  ].parameters[0].value ===
                ...
                <span>
                  ...
                  <Autocomplete
                    ...
                    renderOption={(props, action, state) => {
                      const isParent = getParents(
                        selectedTrigger
                      ).find((parent) => parent.id === action.id);
              ...
              <TextField
                ...
                onBlur={(e) => {
                  ...
                  workflow.triggers[
                    selectedTriggerIndex
                  ].parameters[1].value = e.target.value;
                  setWorkflow(workflow);
                }}
              />
              {!showDropdown ? null : (
          ...
          <div style={{ display: "flex", marginTop: 10 }}>
            <div>
              ...
              <TextField
                ...
                onChange={(event) => {
                  selectedComment.backgroundcolor =
                    event.target.value;
                  setSelectedComment(selectedComment);
                }}
              />
      ...
      if (
        ...
        if (selectedTrigger.environment !== "cloud") {
          if (
            ...
            console.log(
              "Url is wrong. NOT updating because of hybrid."
            );
            //workflow.triggers[selectedTriggerIndex].parameters[0].value = newUrl;
            //setWorkflow(workflow);
          }
            ...
            <Select
              ...
              onChange={(e) => {
                ...
                if (e.target.value === "cloud") {
                  ...
                  const newurl =
                    "https://shuffler.io/" + urlpath.join("/");
                  workflow.triggers[
                    selectedTriggerIndex
                  ].parameters[0].value = newurl;
                } else {
                  const tmpvalue =
                    workflow.triggers[
                      selectedTriggerIndex
            ...
            <TextField
              ...
              helperText={
                ...
                (workflow.triggers[
                  ...
                  >
                    PS: This does NOT work with localhost. Use your
                    local IP instead.
                  </span>
                ) : null
              }
  ...
  const startMailSub = (trigger, triggerindex) => {
    ...
    const splitItem =
      workflow.triggers[
        selectedTriggerIndex
      ].parameters[0].value.split(splitter);
    ...
    for (let splitkey in splitItem) {
      ...
      const curfolder = triggerFolders.find(
        (a) => a.displayName === item
      );
          ...
          <div style={{ flex: "6", marginTop: 10 }}>
            ...
            >
              <div style={{ flex: "10" }}>
                ...
                <Typography variant="body2" color="textSecondary">
                  The information you want to show the user. Supports
                  variables.
                </Typography>
              </div>
            </div>
            ...
            >
              <div style={{ flex: "10" }}>
                <b>Input options</b>
                <Typography variant="body2" color="textSecondary">
                  Use subflows to connect to any app you want, or use
                  the default email and sms options
                </Typography>
              </div>
            </div>
            <FormGroup
                ...
                <Button
                  ...
                  disabled={
                    !(
                      subworkflow === null ||
                      subworkflow === undefined ||
                      Object.getOwnPropertyNames(subworkflow)
                        .length === 0
                    )
                  }
                  onClick={() => {
            ...
            workflow.triggers[
              ...
              <TextField
                ...
                onBlur={(event) => {
                  workflow.triggers[
                    selectedTriggerIndex
                  ].parameters[3].value = event.target.value;
                  setWorkflow(workflow);
                  setUpdate(Math.random());
                }}
              />
            ...
            workflow.triggers[
              ...
              <TextField
                ...
                onBlur={(event) => {
                  workflow.triggers[
                    selectedTriggerIndex
                  ].parameters[4].value = event.target.value;
                  setWorkflow(workflow);
                  setUpdate(Math.random());
                }}
              />
            ...
            <div
              ...
              onClick={() => {
                if (selectedTrigger.status === "running") {
                  toast(
                    "please stop the trigger to edit the configuration"
                  );
                  return;
                } else {
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (bootstrapServers) {
                    ...
                    toast(
                      "please enter the bootstrap servers details"
                    );
                    return;
                  }
          ...
          <div>
            ...
            <TextField
              ...
              disabled={
                ...
                workflow.triggers[selectedTriggerIndex] === undefined
                  ? false
                  : workflow.triggers[selectedTriggerIndex].status ===
                    "running"
              }
              fullWidth
              rows="6"
  ...
  const WorkflowMenu = () => {
    ...
    const [showShuffleMenu, setShowShuffleMenu] =
      React.useState(false);
        ...
        >
          ...
          >
            ...
            <FormControlLabel
              style={{ marginBottom: 15, color: "white" }}
              label={
                <div style={{ color: "white" }}>Exit on Error</div>
              }
              control={
                <Switch
            ...
            <FormControlLabel
              style={{ marginBottom: 15, color: "white" }}
              label={
                <div style={{ color: "white" }}>Start from top</div>
              }
              control={
                <Switch
      ...
      >
        ...
        (userdata.avatar === creatorProfile.github_avatar ||
          ...
          <div style={{ marginTop: 50 }}>
            <Typography variant="body2" color="textSecondary">
              You can see these buttons because you may have the
              correct access rights as a creator to help modify this
              workflow.
            </Typography>
            <Button
              color="primary"
    ...
    if (execution.execution_source === "webhook") {
      ...
      return (
        <img
          ...
          src={
            triggers.find(
              (trigger) => trigger.trigger_type === "EMAIL"
            ).large_image
          }
          style={{
            width: size,
      ...
      return (
        <img
          ...
          src={
            triggers.find(
              (trigger) => trigger.trigger_type === "PIPELINE"
            ).large_image
          }
    ...
    if (
      ...
      return (
        <img
          ...
          src={
            triggers.find(
              (trigger) => trigger.trigger_type === "SUBFLOW"
            ).large_image
          }
          style={{
            width: size,
  ...
  const handleReactJsonClipboard = (copy) => {
    ...
    if (copyText !== null && copyText !== undefined) {
      ...
      if (stringified.startsWith('"') && stringified.endsWith('"')) {
        stringified = stringified.substring(
          1,
      ...
      console.log(
        "Failed to copy from " + elementName + ": ",
        copyText
      );
    }
  };
  ...
  const HandleJsonCopy = (base, copy, base_node_name) => {
    ...
    to_be_copied =
      "$" + base_node_name.toLowerCase().replaceAll(" ", "_");
  ...
  const ShowReactJsonField = (props) => {
    const { validate, jsonValue, collapsed, label, autocomplete } =
      props;

    const [parsedCollapse, setParsedCollapse] =
  ...
  const executionModal = (
    ...
    >
      ...
      {executionModalView === 0 ? (
        <div
          style={{
            padding: isMobile ? "0px 0px 0px 10px" : 25,
            zIndex: 12502,
          }}
        >
          <div style={{ display: "flex" }}>
          ...
          {workflowExecutions.length > 0 ? (
              ...
              {workflowExecutions.map((data, index) => {
                ...
                if (
                  ...
                  for (let triggerkey in data.workflow.triggers) {
                    const trigger =
                      data.workflow.triggers[triggerkey];
                    if (
                      (trigger.app_name === "User Input" &&
                        trigger.trigger_type === "USERINPUT") ||
                      (trigger.app_name === "Shuffle Workflow" &&
                      ...
                      >
                        <Paper
                          ...
                          onClick={() => {
                            ...
                            if (
                              ...
                              if (
                                data.execution_argument !==
                                  undefined &&
                                data.execution_argument !== null &&
                                data.execution_argument.includes(
                                  "too large"
                                )
                              ) {
                                setExecutionData({});
                            ...
                            if (!checkStarted) {
                              ...
                              if (cy !== undefined && cy !== null) {
                                ...
                                for (let actionKey in data.workflow
                                  .actions) {
                                  var actionitem =
                        ...
                        >
                          <div style={{ display: "flex", flex: 1 }}>
                            ...
                            <Tooltip
                              color="primary"
                              title={`Execution was ran with a ${
                          ...
                          {foundnotifications > 0 ? (
                            <Tooltip
                              title={
                                ...
                                foundnotifications +
                                " notification(s). Click to explore them."
                              }
                              placement="top"
                            >
                              <ErrorOutlineIcon
            ...
            <div>
              ...
              >
                Test Workflow{" "}
                <PlayArrowIcon style={{ marginLeft: 15 }} />
              </Button>
            </div>
          )}
        </div>
        ...
        >
          ...
          executionData.completed_at > 0 ? (
            <div style={{ display: "flex", marginLeft: 10 }}>
              ...
              <Typography variant="body1" color="textSecondary">
                {new Date(
                  executionData.completed_at * 1000
                ).toLocaleString("en-GB")}
              </Typography>
            </div>
          ) : null}
                ...
                <div style={{}}>
                  ...
                  environments[defaultEnvironmentIndex].Name !==
                    ...
                    >
                      ...
                      . If the Workflow doesn't start within 30
                      seconds with Orborus running, contact support:{" "}
                      <a
          ...
          (executionData.results.length === 0 &&
            ...
            <div style={{}}>
              ...
              environments[defaultEnvironmentIndex].Name !==
                ...
                >
                  ...
                  . If the Workflow doesn't start within 30 seconds
                  with Orborus running, contact support:{" "}
                  <a
                    href="mailto:support@shuffler.io"
                    rel="noopener noreferrer"
                    target="_blank"
            ...
            executionData.results.map((data, index) => {
              ...
              var imgSrc =
                curapp === undefined ? "" : curapp.large_image;
              if (
                imgSrc.length === 0 &&
                workflow.actions !== undefined &&
                workflow.actions !== null
                ...
                if (
                  ...
                  for (let actionkey in workflow.actions) {
                    if (
                      ...
                      workflow.actions[actionkey].id ===
                        data.action.id ||
                      workflow.actions[actionkey].label ===
                        data.action.label ||
                      workflow.actions[actionkey].name ===
                        data.action.name
                    ) {
                      if (
                        workflow.actions[actionkey].large_image !==
                          undefined &&
                        workflow.actions[actionkey].large_image !==
                          null &&
                        workflow.actions[actionkey].large_image
                ...
                curapp === null ? null : (
                  <img
                    ...
                    style={{
                      ...
                      borderRadius:
                        executionData.start === data.action.id
                          ? 25
                          : 5,
                    }}
                  />
              ...
              if (
                ...
                if (
                  ...
                  try {
                    validate.result.result = JSON.parse(
              ...
              if (
                ...
                if (
                  ...
                  for (let [k, kval] in Object.entries(
                    data.similar_actions
                  )) {
                    if (data.similar_actions.hasOwnProperty(k)) {
                      if (
                        data.similar_actions[k].similarity >
                        minimumMatch
                      ) {
                  ...
                  similarActionsView = (
                    ...
                    >
                      <IconButton
                        ...
                        onClick={() => {
                          navigate(
                            `?execution_highlight=${parsed_url}`
                          );
                        }}
                      >
                        <PreviewIcon
              ...
              const cursearch =
                ...
                window.location === undefined
                  ? ""
                  : window.location.search;
              const chosenNodeId = new URLSearchParams(cursearch).get(
                "node"
              );
              const highlightNode =
                chosenNodeId !== null &&
              ...
              return (
                <div
                  ...
                  onMouseOver={() => {
                    ...
                    var currentnode = cy.getElementById(
                      data.action.id
                    );
                    if (
                      currentnode !== undefined &&
  ...
  const getErrorSuggestion = (result) => {
    ...
    if (stringjson.includes("invalidurl")) {
      ...
      var additionalinfo = "";
      if (
        stringjson.includes("http") &&
        stringjson.match(/http/g).length > 1
      ) {
        additionalinfo =
          "You may be using multiple 'http' in the URL. ";
      }
    ...
    if (
      isCloud &&
      stringjson.toLowerCase().includes("timeout error")
    ) {
          ...
          <IconButton
            ...
            onClick={(e) => {
              ...
              const executionIndex = workflowExecutions.findIndex(
                (data) =>
                  data.execution_id === selectedResult.execution_id
              );
      ...
      <div style={{ marginBottom: 40 }}>
            ...
            <img
              ...
              src={
                selectedResult === undefined
                  ? theme.palette.defaultImage
                  : selectedResult.action.app_name ===
                    "shuffle-subflow"
              ...
              style={{
                ...
                border: `2px solid ${statusColor}`,
                filter:
                  curapp === undefined ? "grayscale(100%)" : null,
              }}
            />
          )}
        ...
        {validate.valid ? (
          <ReactJson
            ...
            style={theme.palette.reactJsonStyle}
            collapsed={
              selectedResult.result.length < 10000 ? false : true
            }
            iconStyle={theme.palette.jsonIconStyle}
            collapseStringsAfterLength={
              theme.palette.jsonCollapseStringsAfterLength
          ...
          selectedResult.action.parameters !== undefined ? (
            <div>
              ...
              {selectedResult.action.parameters.map((data, index) => {
                ...
                return <AppResultVariable key={index} data={data} />;
              })}
            </div>
          ) : null}
        </div>
  ...
  const newView = (
    <div style={{ color: "white" }}>
      <div
        style={{
          display: "flex",
          borderTop: "1px solid rgba(91, 96, 100, 1)",
        }}
      >
  ...
  const ExecutionVariableModal = (props) => {
    ...
    const [newVariableDescription, setNewVariableDescription] =
      React.useState(
        variableInfo.description !== undefined
    ...
    return (
      ...
      >
        <FormControl>
          ...
          <DialogContent>
            Runtime Variables are TEMPORARY variables that you can
            only be set and used during execution. Learn more{" "}
            <a
              rel="noopener noreferrer"
              href="https://shuffler.io/docs/workflows#execution_variables"
              target="_blank"
              ...
              onClick={() => {
                ...
                if (found !== -1) {
                  if (newVariableName.length > 0) {
                    workflow.execution_variables[found].name =
                      newVariableName;
                  }
            ...
            <Button
              ...
              onClick={() => {
                ...
                if (!handled) {
                  ...
                  if (found !== -1) {
                    if (newVariableName.length > 0) {
                      workflow.workflow_variables[found].name =
                        newVariableName;
                    }
                    if (newVariableDescription.length > 0) {
  ...
  const AuthenticationData = (props) => {
    ...
    const handleSubmitCheck = () => {
      ...
      for (let paramkey in selectedApp.authentication.parameters) {
        if (
          ...
          if (
            ...
            if (
              selectedApp.authentication.parameters[paramkey].schema
                .type === "bool"
            ) {
              authenticationOption.fields[
                selectedApp.authentication.parameters[paramkey].name
              ...
              toast(
                "Field " +
                  selectedApp.authentication.parameters[paramkey]
                    .name +
                  " can't be empty"
              );
              return;
      ...
      var newAuthOption = JSON.parse(
        JSON.stringify(authenticationOption)
      );
      var newFields = [];
    ...
    if (
      ...
      authenticationOption.label =
        selectedApp.name + " authentication";
    }
    ...
    return (
      <div>
        ...
        >
          <div style={{ color: "white" }}>
            Authentication for{" "}
            {selectedApp.name.replaceAll("_", " ", -1)}
          </div>
        </DialogTitle>
        <DialogContent>
          <a
          ...
          <div />
          These are required fields for authenticating with{" "}
          {selectedApp.name}
          <div style={{ marginTop: 15 }} />
          ...
          />
          <div />
          {selectedApp.authentication.parameters.map(
            (data, index) => {
              return (
          ...
          selectedApp.documentation.length === 0 ? (
            <span style={{ textAlign: "center" }}>
              ...
              <Typography variant="h6">
                There is currently no extended documentation available
                for this app.
              </Typography>
          ...
          {selectedOption === "Kafka Queue" ? (
            <div>
              ...
              <TextField
                ...
                color="primary"
                placeholder={
                  "broker1.example.com:9092,192.168.1.100:9092"
                }
                defaultValue={
                  selectedTrigger?.parameters?.find(
                    (param) => param.name === "bootstrap_servers"
  ...
  const aiSubmit = (
    ...
    if (inputAction !== undefined) {
      console.log(
        "Add app context! This should them get parameters directly"
      );
      ...
      if (!value.includes(inputAction.label)) {
        conversationData.query = inputAction.label.replaceAll(
          "_",
          " "
        );
      }
          ...
          for (let paramkey in inputAction.parameters) {
            ...
            for (let respParam of responseJson.parameters) {
              if (respParam.name === actionParam.name) {
                ...
                inputAction.parameters[paramkey].value =
                  respParam.value;
                break;
              }
            }
    ...
    return (
      ...
      >
        ...
        >
          ...
          >
            <CloseIcon
              style={{ color: "white", height: 12, width: 12 }}
            />
          </IconButton>
        </Tooltip>
        <form
  ...
  const RevisionBox = (props) => {
    ...
    const validTimestamp =
      newrevision.edited.toString().length === 10
        ...
        : newrevision.edited;
    const translatedDate = new Date(validTimestamp).toLocaleString(
      "en-US",
      {
        month: "long",
        day: "numeric",
    ...
    return (
      <Paper
        style={{
          ...
          cursor: "pointer",
          backgroundColor:
            newrevision.edited === selectedRevision.edited
      ...
      >
        <div style={{ display: "flex" }}>
          <span style={{ flex: 5 }}>
            <Typography variant="body1">{translatedDate}</Typography>
          </span>
          <span style={{ flex: 2 }}>
            <Chip
        ...
        {/*revision.edited === originalWorkflow.edited ?
				...
				: null*/}
        <div style={{ display: "flex" }}>
          {revision.actions !== undefined &&
          revision.actions !== null ? (
            <Tooltip
              color="primary"
          ...
          {revision.triggers !== undefined &&
          revision.triggers !== null ? (
            <Tooltip
              color="primary"
  ...
  const drawerData =
    ...
    originalWorkflow !== {} ? (
      <div style={{ padding: "0px 25px 100px 25px" }}>
        ...
        <RevisionBox revision={originalWorkflow} />
        ...
        {allRevisions.length > 0 ? (
          ...
          <div style={{ padding: 5 }}>
            <Typography variant="body2">
              No other revisions found. Save your workflow with
              changes to create a revision.
            </Typography>
          </div>
        )}
      </div>
  ...
  const changeActionParameterCodeMirror = (
    event,
    count,
    data,
    actionlist
    ...
    if (
      ...
      if (curstring.length > 0 && actionlist !== null) {
        ...
        var actionItem = actionlist.find(
          (data) =>
            data.autocomplete.split(" ").join("_").toLowerCase() ===
            curstring
        );
  ...
  if (
    ...
    const rightSideActionView = document.getElementById(
      "rightside_actions"
    );
    if (
      rightSideActionView !== undefined &&
      rightSideActionView !== null
    ) {
      if (
