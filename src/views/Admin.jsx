...
const Admin = (props) => {
  ...
  const [
    ...
    setAppAuthenticationGroupModalOpen,
  ] = React.useState(false);
  const [appsForAppAuthGroup, setAppsForAppAuthGroup] =
    React.useState([]);
  const [appAuthenticationGroupName, setAppAuthenticationGroupName] =
    React.useState("");
  const [
    appAuthenticationGroupDescription,
    setAppAuthenticationGroupDescription,
  ] = React.useState("");
  const [appAuthenticationGroups, setAppAuthenticationGroups] =
    React.useState([]);
  ...
  const [matchingOrganizations, setMatchingOrganizations] =
    React.useState([]);
  ...
  const [selectedAuthentication, setSelectedAuthentication] =
    React.useState({});
  const [
    selectedAuthenticationModalOpen,
    setSelectedAuthenticationModalOpen,
  ] = React.useState(false);
  const [authenticationFields, setAuthenticationFields] =
    React.useState([]);
  ...
  const [showDeleteAccountTextbox, setShowDeleteAccountTextbox] =
    React.useState(false);
  const [deleteAccountText, setDeleteAccountText] =
    React.useState("");
  ...
  const setSelectedRegion = (region) => {
    ...
    fetch(
      `${globalUrl}/api/v1/orgs/${selectedOrganization.id}/change/region`,
      {
        method: "POST",
        headers: {
      ...
      if (response.status !== 200) {
        ...
        toast(
          "Region changed successfully! Reloading in 5 seconds.."
        );
  ...
  const createAppAuthenticationGroup = (
    name,
    description,
    appAuthIds
  ) => {
    ...
    fetch(globalUrl + "/api/v1/apps/authentication/group", {
      ...
      .then((response) => {
        if (response.status !== 200) {
          throw new Error(
            "Failed to create app authentication group"
          );
        }
...
If you're interested, please let me know a time that works for you, or set up a call here: https://drift.me/${username}`;
  ...
  const changeDistribution = (data) => {
    ...
    console.log(
      "Should change distribution to be shared among suborgs"
    );
  ...
  const startSchedule = (trigger) => {
    ...
    fetch(
      `${globalUrl}/api/v1/workflows/${trigger.workflow_id}/schedule`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const changePipelineState = (pipeline, state) => {
    ...
    fetch(url, {
      ...
      .then((responseJson) => {
        if (!responseJson.success) {
          toast(
            "Failed to update the pipeline: " + responseJson.reason
          );
        } else {
          if (state === "start")
      ...
      .then((responseJson) => {
        ...
        if (
          ...
          if (disableSync) {
            ...
            setOrgSyncResponse(
              "Successfully disabled syncronization"
            );
          } else {
      ...
      .then((response) =>
        response.json().then((responseJson) => {
          if (responseJson["success"] === false) {
            ...
            if (responseJson.reason !== undefined) {
              toast(
                "Failed changing authentication: " +
                  responseJson.reason
              );
            } else {
  ...
  const editAuthenticationConfig = (id, parentAction) => {
    ...
    const url =
      globalUrl + "/api/v1/apps/authentication/" + id + "/config";
  ...
  const createSubOrg = (currentOrgId, name) => {
    ...
    console.log(data);
    const url =
      globalUrl + `/api/v1/orgs/${currentOrgId}/create_sub_org`;
  ...
  const rerunCloudWorkflows = (environment) => {
    toast(
      "Starting execution reruns. This can run in the background."
    );
    fetch(
  ...
  const deleteEnvironment = (environment) => {
    ...
    for (var key in environments) {
      if (environments[key].id == id) {
        ...
        if (
          environments[key].type === "cloud" &&
          !environments[key].archived
        ) {
          toast("Can't modify cloud environments");
          return;
        }
    ...
    >
      <DialogTitle>
        ...
        <Typography
          ...
          style={{ marginTop: 10 }}
        >
          You can <b>not</b> see the previous values for an
          authentication while editing. This is to keep your data
          secure. You can overwrite one- or multiple fields at a time.
        </Typography>
      </DialogTitle>
      <DialogContent>
        ...
        selectedAuthentication.type === "oauth2-app" ? (
          <div>
            ...
            >
              Only the name and url can be modified for Oauth2/OpenID
              connect. Please remake the authentication if you want to
              change the other fields like Client ID, Secret, Scopes
              etc.
            </Typography>
          </div>
        ) : null}
        ...
        >
          {selectedOrganization.child_orgs.map((org, index) => (
            <MenuItem key={index} value={org.id}>
              <Checkbox
                checked={matchingOrganizations.indexOf(org.id) > -1}
              />
  ...
  const editUserModal = (
    ...
    >
      <DialogTitle style={{ maxWidth: 450, margin: "auto" }}>
        <span style={{ color: "white" }}>
          <EditIcon style={{ marginTop: 5 }} /> Editing{" "}
            ...
            secret2FA.length > 0 ? (
              <span>
                <Typography variant="body2" color="textSecondary">
                  ...
                  authentication app on your phone. If you can’t use a
                  QR code, use the code {secret2FA} instead.
                </Typography>
              </span>
            ) : null}
            {image2FA !== undefined &&
        ...
        >
          ...
          >
            ...
            <ListItemText
              ...
              primary={
                props.data.newname ? props.data.newname : primary
              }
            />
      ...
      <div style={{ position: "relative" }}>
        <div style={{ marginTop: 20, marginBottom: 20 }}>
          ...
          <Typography
            ...
            style={{ marginLeft: 0 }}
          >
            On this page organization admins can configure
            organizations, and sub-orgs (MSSP).{" "}
            <a
              target="_blank"
              rel="noopener noreferrer"
          ...
          <div>
            ...
            {adminTab === 0 ? (
              ...
              <div>
                ...
                do? Cloud syncronization is a way of getting more out
                of Shuffle. Shuffle will <b>ALWAYS</b> make every
                option open source, but features relying on other
                users can't be done without a collaborative approach.
                {isCloud ? (
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {selectedOrganization.cloud_sync_active ? (
                        <Typography style={{}}>
                          ...
                          {selectedOrganization.sync_config
                            .interval === 0
                            ? "60"
                            : selectedOrganization.sync_config
                                .interval}
                ...
                >
                  Features and Limitations that are currently
                  available to you in your Cloud or Hybrid
                  Organization. App Executions (App Runs) reset
                  monthly. If the organization is a customer or in a
                  trial, these features limitations are not always
                  enforced.
                </Typography>
                <Grid
                  container
                ...
                >
                  ...
                  selectedOrganization.sync_features === null
                    ? null
                    : Object.keys(
                        selectedOrganization.sync_features
                        ...
                        const item =
                          selectedOrganization.sync_features[key];
    ...
    >
      ...
      <DialogContent>
        {curTab === 1 && isCloud ? (
          ...
          <Typography variant="body1" style={{ marginBottom: 10 }}>
            The organization created will become a child of your
            current organization, and be available to you.
          </Typography>
        ) : null}
        {curTab === 1 ? (
          <div>
              ...
              <FormControl fullWidth>
                ...
                >
                  {(() => {
                    ...
                    return userLogViewing.login_info.map(
                      (data, index) => {
                        if (
    ...
    curTab === 5 ? (
      <div>
        <div style={{ marginTop: 20, marginBottom: 20 }}>
          ...
          <span style={{ marginLeft: 25 }}>
            Schedules used in Workflows. Makes locating and control
            easier.{" "}
            <a
              target="_blank"
              rel="noopener noreferrer"
    ...
    curTab === 8 ? (
      <div>
        <div style={{ marginTop: 20, marginBottom: 20 }}>
          ...
          <span style={{ marginLeft: 25 }}>
            Categories are the categories supported by Shuffle, which
            are mapped to apps and workflows
          </span>
        </div>
        <Divider
          style={{
            ...
            return (
              <ListItem key={index}>
                ...
                >
                  ...
                  >
                    Find app (
                    {data.apps === null ? 0 : data.apps.length})
                  </Button>
                </ListItemText>
              </ListItem>
            );
                          ...
                          <Tooltip title={data.app.name}>
                            ...
                            >
                              ...
                              <Checkbox
                                ...
                                name={data.label}
                                disabled={
                                  data.app.id in appsForAppAuthGroup
                                }
                              />
                            </div>
                          </Tooltip>
                    ...
                    data.fields = [
                      {
                        key: "url",
                        value:
                          "Secret. Replaced during app execution!",
                      },
                      {
                        key: "client_id",
                        value:
    ...
    curTab === 6 ? (
      <div>
        <div style={{ marginTop: 20, marginBottom: 20 }}>
          ...
          <span style={{ marginLeft: 25 }}>
            Decides what Orborus environment to run your workflow
            actions. If you have scale problems, talk to our team:
            support@shuffler.io.&nbsp;
            <a
              target="_blank"
              rel="noopener noreferrer"
        ...
        <List>
          ...
          {environments === undefined || environments === null
            ...
            : environments.map((environment, index) => {
                ...
                const queueSize =
                  environment.queue !== undefined &&
                  environment.queue !== null
                    ? environment.queue < 0
                      ? 0
                            ...
                            <IconButton
                              ...
                              onClick={() => {
                                ...
                                if (
                                  ...
                                  toast(
                                    "Orborus command copied to clipboard"
                                  );
                                }
                              }}
                            >
                        ...
                        >
                          ...
                          >
                            <Typography variant="body1">
                              90% CPU the server(s) hosting the
                              Shuffle App Runner (Orborus) was found.
                            </Typography>
                            <Typography
                              variant="body2"
                              color="textSecondary"
                            >
                              Need help with High Availability and
                              Scale?{" "}
                              <a
                ...
                <List>
                  ...
                  <ListItem style={{ backgroundColor: bgColor }}>
                    ...
                    <Button
                      ...
                      color="primary"
                      disabled={
                        parentOrg.id === selectedOrganization.id
                      }
                      onClick={() => {
